...
function _readGoInfo(_f:stdgo._internal.io.Io_reader.Reader, _info:stdgo.Ref<stdgo._internal.go.build.Build_t_fileinfo.T_fileInfo>):stdgo.Error {
        ...
        for (__0 => _decl in (@:checkr (@:checkr _info ?? throw "null pointer dereference")._parsed ?? throw "null pointer dereference").decls) {
            var __tmp__ = try {
            ...
            for (__1 => _dspec in (@:checkr _d ?? throw "null pointer dereference").specs) {
                var __tmp__ = try {
        ...
        for (__1 => _group in (@:checkr (@:checkr _info ?? throw "null pointer dereference")._parsed ?? throw "null pointer dereference").comments) {
            ...
            for (__2 => _c in (@:checkr _group ?? throw "null pointer dereference").list) {
